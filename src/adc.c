...
static void update_ring(uint8_t type, int16_t value) {
  ring_sum[type] += value;
  ring_sum[type] -=
      ring_buf[type][ring_pos]; // note for boot: it is OK, ring_buf is
...
static int16_t ADC_Convert_To_Degree(int16_t adc) {
   ...
   */
  dummy = (int16_t)((((int32_t)(adc - kx)) * (-TEMP_CAL_STEP)) /
                    (int32_t)(kx_d[i]));
...
bool task_ADC(void) {
  ...
  case 1: // step 1
    // set ADC control and status register
    ADCSRA =
        (1 << ADEN) | (1 << ADPS2) | (1 << ADPS0) | (1 << ADIE); // prescaler=32
...
#if !TASK_IS_SFR
// not optimized
ISR(ADC_vect) { task |= TASK_ADC; }
#else
// optimized
ISR_NAKED ISR(ADC_vect) {
  /* note: __zero_reg__ is not used, It can be reused on some other user
   * assembler code */
