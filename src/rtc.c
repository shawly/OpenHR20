...
rtc_t RTC = {BOOT_YY, BOOT_MM, BOOT_DD, BOOT_hh, BOOT_mm, 0, 0};
...
const uint8_t RTC_DayOfMonthTablePrgMem[] PROGMEM = {31, 28, 31, 30, 31, 30,
                                                     31, 31, 30, 31, 30, 31};
 ...
 ******************************************************************************/
void RTC_SetHour(int8_t hour) { RTC.hh = (uint8_t)(hour + 24) % 24; }
 ...
 ******************************************************************************/
void RTC_SetMinute(int8_t minute) { RTC.mm = (uint8_t)(minute + 60) % 60; }
 ...
 ******************************************************************************/
void RTC_SetSecond(int8_t second) { RTC.ss = (uint8_t)(second + 60) % 60; }
 ...
 ******************************************************************************/
void RTC_SetSecond100(uint8_t second100) { RTC_s100 = (second100 + 100) % 100; }
#endif
 ...
 *
 ******************************************************************************/
bool RTC_DowTimerSet(rtc_dow_t dow, uint8_t slot, uint16_t time,
...
int32_t RTC_DowTimerGetHourBar(uint8_t dow) {
  ...
  while (time > 0) {
    ...
    uint16_t table_time =
        ((raw_idx >= 0) ? eeprom_timers_read_raw(raw_idx) : 0);
...
ISR_NAKED ISR(TIMER2_OVF_vect) {
  asm volatile("__my_tmp_reg__ = 16"
               "\n"
               /* prologue */
               "	push __my_tmp_reg__"
               "\n"
               ...
               : "I"(_SFR_IO_ADDR(task)), "I"(TASK_RTC_BIT),
                 "M"(_BV(RTC_TIMER_OVF) | _BV(RTC_TIMER_RTC)));
}
#endif
...
ISR(TIMER2_COMP_vect) {
  ...
  COM_putchar('%');
#endif
  if ((RTC_timer_todo & _BV(RTC_TIMER_KB)) &&
      (t2 == RTC_timer_time[RTC_TIMER_KB - 1])) {
 ...
 *
 ******************************************************************************/
#define EXTERNAL_TICKS                                                         \
  200 // ticks on XTAL. Modify to increase/decrease accuracy
...
void calibrate_rco(void) {
  ...
  static const uint16_t countVal =
      (EXTERNAL_TICKS * F_CPU) / (XTAL_FREQUENCY * LOOP_CYCLES);
