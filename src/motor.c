...
static volatile motor_dir_t
    MOTOR_Dir_Counter; //!< actual direction for count pulses
...
#pragma GCC diagnostic ignored "-Wstrict-aliasing"
      if (cal_type == 3) {
        ...
        eeprom_config_save((uint16_t)(&config.MOTOR_ManuCalibration_L) -
                           (uint16_t)(&config));
        eeprom_config_save((uint16_t)(&config.MOTOR_ManuCalibration_H) -
                           (uint16_t)(&config));
      } else if (cal_type == 2) {
...
static void MOTOR_Control(motor_dir_t direction) {
  ...
  } else // motor on
  {
    if (MOTOR_Dir != direction) {
      ...
      motor_max_time_for_impulse =
          ((uint16_t)config.motor_speed *
           ((MOTOR_IsCalibrated()) ? (uint16_t)config.motor_end_detect_run
                                   : (uint16_t)config.motor_end_detect_cal) /
           100)
          << 3;
      ...
      PCMSK0 |= (1 << PCINT4); // enable interrupt from eye
      {
        ...
        MOTOR_pwm_set((int16_t)(((uint16_t)config.motor_pwm_max * 256) /
                                ((bat_average) / (2800 / 256))));
#else
...
void MOTOR_timer_pulse(void) {
  if (motor_diag_ignore == 0) {
    {
      int16_t chg =
          (((int16_t)((motor_diag + 4) >> 3) - (int16_t)config.motor_speed) *
           config.motor_speed_ctl_gain) /
          100;
...
#pragma GCC diagnostic ignored "-Wstrict-aliasing"
          ...
          } else if (MOTOR_ManuCalibration >
                     0) // already calibrated, reuse eeprom data
          {
