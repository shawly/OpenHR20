...
uint8_t CTL_temp_wanted_last =
    0xff; // desired temperature value used for last PID control
uint8_t CTL_temp_auto_type =
    TEMP_TYPE_INVALID;       // actual desired temperature type by timer
...
#endif
static uint16_t PID_update_timeout =
    AVERAGE_LEN +
    1; // timer to next PID controler action/first is 16 sec after statup
int8_t PID_force_update =
    AVERAGE_LEN +
    1; // signed value, val<0 means disable force updates \todo rename
uint8_t valveHistory[VALVE_HISTORY_LEN];

static uint8_t pid_Controller(int16_t setPoint, int16_t processValue,
                              uint8_t old_result, bool updateNow);

uint8_t CTL_error = 0;

__attribute__((noinline)) // do not inline in this file
void
CTL_set_error(int8_t err_code)
{
...
__attribute__((noinline)) // do not inline in this file
void
CTL_clear_error(int8_t err_code)
{
    int8_t old_err = CTL_error;
...
static void CTL_window_detection(void)
{
    ...
    window_timer = (w) ? (config.window_close_detection_delay)
                       : (config.window_open_detection_delay);
}
#else
static void CTL_window_detection(void)
{
    uint8_t i = (ring_buf_temp_avgs_pos + AVGS_BUFFER_LEN -
                 ((CTL_mode_window != 0) ? config.window_close_detection_time
                                         : config.window_open_detection_time)) %
                AVGS_BUFFER_LEN;
    int16_t min = 10000;
    int16_t max = 0;
    ...
    {
        if ((CTL_mode_window == 0) &&
            ((max - temp_average) > (int16_t)config.window_open_detection_diff))
        {
...
void CTL_update(bool minute_ch)
{
    ...
    if (minute_ch || (CTL_temp_auto_type == TEMP_TYPE_INVALID))
    {
        ...
        uint8_t t = RTC_ActualTimerTemperatureType(
        ...
        if (t != TEMP_TYPE_INVALID)
        {
            ...
            if (CTL_mode_auto)
            {
                ...
                if ((PID_force_update < 0) &&
                    (CTL_temp_wanted != CTL_temp_wanted_last))
                {
                    PID_force_update = 0;
                }
            }
    ...
    else if ((PID_update_timeout == 0) || (PID_force_update == 0))
    {
        ...
        if (updateNow || (PID_update_timeout == 0))
        {
            ...
            CTL_temp_wanted_last = temp;
            {
                ...
                {
                    CTL_integratorBlock =
                        DEFINE_INTEGRATOR_BLOCK; // block Integrator if valve moves
                }
#endif

                for (i = VALVE_HISTORY_LEN - 1; i > 0; i--)
                {
                    if (updateNow || (new_valve <= config.valve_max) ||
                        (new_valve >= config.valve_min))
                    {
...
void CTL_change_mode(int8_t m)
{
    ...
    if (CTL_mode_auto && (m != CTL_CHANGE_MODE_REWOKE))
    {
        ...
        CTL_temp_wanted = (CTL_temp_auto_type != TEMP_TYPE_INVALID
                               ? temperature_table[CTL_temp_auto_type]
                               : 0);
...
static void testIntegratorRevert(uint16_t absErr)
{
    ...
    {
        // if error could not be reduced to 3/4 and Error is larger than
        // I_ERR_TOLLERANCE_AROUND_0°C
        sumError = lastTempChangeSumError;
    }
    lastTempChangeErrorAbs =
        0xffff; // function can be called more time but only first is valid
}
                              ...
                              uint8_t old_result, bool updateNow)
{
...

    {
        ...
        if (updateNow)
        {
            ...
            CTL_integratorBlock =
                DEFINE_INTEGRATOR_BLOCK; // do not allow update integrator immediately
                                         // after temp change
            testIntegratorRevert(lastAbsError);
            lastTempChangeErrorAbs = absErr;
            lastTempChangeSumError = sumError;
#if BOOST_CONTROLER_AFTER_CHANGE
            if ((abs(setPoint - CTL_temp_wanted_last) >=
                 config.temp_boost_setpoint_diff) // change of wanted temp large
                                                  // enough to start boost (0,5°C)
                &&
                (absErr >= (int16_t)config.temp_boost_error)) // error large enough to
                                                              // start boost (0,3°C)
            {
                PID_boost_timeout = (error16 >= 0) ? config.temp_boost_time_heat
                                                   : config.temp_boost_time_cool;
                PID_boost_timeout = (uint8_t)(MIN(
                    255,
                    abs(error16) / 10 * (int16_t)PID_boost_timeout /
                        (int16_t)config
                            .temp_boost_tempchange)); // boosttime=error/10(0,1°C)*time/tempchange
            }
#endif
        }
        else
        {
            if (CTL_integratorBlock == 0)
            {
                ...
                if ((error16 >= 0) ? (old_result < config.valve_max)
                                   : (old_result > config.valve_min))
                {
                    if (((lastErrorSign != ((uint8_t)(error16 >> 8) & 0x80))) ||
                        ((absErr == last2AbsError) &&
                         (absErr <=
                          I_ERR_TOLLERANCE_AROUND_0))) // sign of last error16 != sign of
                                                       // current OR abserror around 0
                    {
                        CTL_interatorCredit = config.I_max_credit; // ? optional
                        CTL_creditExpiration = config.I_credit_expiration;
                        goto INTEGRATOR; // next integration, do not change
                                         // CTL_interatorCredit
                    }
                    if (CTL_interatorCredit > 0)
                    {
                        ...
                        {
                            CTL_interatorCredit -=
                                (absErr / I_ERR_WEIGHT) + 1; // max is 1200/20+1 = 61
                        INTEGRATOR:
                            sumError += error16 * 8;
                        }
                    }
    ...
    {
        if ((abs(error16) <=
             (int16_t)(config.temp_boost_error -
                       config
                           .temp_boost_hystereses))) // error <=
                                                     // temp_boost_error-temp_boost_hystereses
        {
    ...
    {
        int32_t maxSumError;
        // for overload protection: maximum is scalling_factor*scalling_factor*50/1
        // = 3276800
        maxSumError = ((int32_t)scalling_factor * (int32_t)scalling_factor * 50) /
                      config.I_Factor;
    ...
    pi_term *= (int32_t)error16;
    pi_term +=
        (int32_t)(config.I_Factor) *
        sumError; // maximum is
                  // 65536*50=(scalling_factor*scalling_factor*50/I_Factor)*I_Factor
    /*
