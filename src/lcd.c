...
#define LCD_MAX_CHARS                                                          \
  (sizeof(LCD_CharTablePrgMem)) //!< \brief no. of chars in \ref
                                //!< LCD_CharTablePrgMem
#ifdef HR25
...
// Vars
volatile uint8_t LCD_used_bitplanes =
    1; //!< \brief number of used bitplanes / used for power save
...
void LCD_PrintChar(uint8_t value, uint8_t pos, uint8_t mode) {
  ...
  if ((pos < LCD_MAX_POS) && (value < LCD_MAX_CHARS)) {
    ...
    for (i = 0; i < 7; i++) {
      seg =
          fieldbase +
          pgm_read_byte(&LCD_SegOffsetTablePrgMem[LCD_upside_down ? 6 - i : i]);
      // Set or Clear?
